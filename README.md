...
```
make dataset       # runs src/dataset.py against data/genres_original/
```
...
| `make dataset` | Runs `src/dataset.py` to generate the spectrogram dataset from `data/genres_original/`. See `docs/dataset.md` for how it works. |
...
│   └── dataset.py        # generates spectrograms/ from data/genres_original/
